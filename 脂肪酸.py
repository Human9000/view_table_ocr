...
def split_col(head):
    ...
    mid = []
    mask = np.zeros((w,))
    mask[int(w * 0.3):int(w * 0.8)] = 1
    x = mean01_dim(head, dim=1, left=0.50, right=0.60, mask_zeros=mask)

    index = np.argmin(x) + 10
    head1 = cv.erode(head[index:-20], np.ones((17, 17), np.uint8), )
    x = min01_dim(head1, 0)
    if DEBUG:
        plt.plot(x)
        plt.show()
    ...
    l = []
    r = []
    for i in range(165, len(x) - 1):
        if x[i] < 0.5 < x[i + 1]:
            r.append(i)
        if x[i] > 0.5 > x[i + 1]:
            l.append(i)
    d = int((l[1] - r[0]) * 0.1)  # 第一列
    mid.append(l[0] - d)
...
def split_row(id_rol):
    ...
    res = [i-20 for id, i in enumerate(index) if abs( i - index[id-1] )> 10]
...
if __name__ == '__main__':
    ...
    for i in os.listdir(root):
        ...
        df.to_csv(f"o3/{page}.tsv", index=False, sep='\t')
        print(df)
